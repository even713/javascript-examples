(function() {
function debugA(nodes){
	var tempArray = nodes.map(function(d){
		var n = {x: d.x, y: d.y, d: d.d, risk: d.risk};
		return n;
	});
	console.log(tempArray);
};

function debugB(nodes){
	var tempArray = nodes.map(function(d){
		var n = {tx: d.tx, ty: d.ty, d: d, risk: d.risk};
		return n;
	});
	console.log(tempArray);
}

var debugNodes = debugB;
	...
	pan.drawview = function(m) {
		...
		nodes.forEach(function(d,i) {
			...
			if (pan.strata&&mode<3) {
				...
				var tScale = d3.scale.linear().range([d.ty-d.max/2,d.ty+d.max/2]).domain([5,0]);
				nodes[i].y = tScale(+d.risk||0) || d.ty;
				console.log("setting y", {"ty": d.ty, "max": d.max, "risk": d.risk, "y": nodes[i].y});
				//console.log("init tScale", d.ty, d.max, d.risk, nodes[i].y);
		...
		// till here, the circle is drawed line by line group by color
